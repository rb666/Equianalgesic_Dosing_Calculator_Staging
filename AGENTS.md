...
The owner authorized staging to temporarily open the eight-logo switcher. `staging/release.json` enables it, and `staging/logo-preview/` contains the explicit preview source/assets. `/opioidcalculator/` is the wrapper, `/opioidcalculator/site/` its working calculator, and `/logo-preview/` the gallery. The mobile-size toggle is removed. Logo 02 is New, not a previous production identity. Original logo 06 is the default. The public calculator source and clinical data remain unchanged. See `staging/logo-preview/README.md` for structure and restoration; disabling the flag and redeploying restores the normal calculator-only artifact. Do not deploy the local experiment folders. This temporary exception applies only to the staging Pages build, never production.
