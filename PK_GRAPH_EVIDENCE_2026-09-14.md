...
At the owner's request, the graph cards now keep the graph, its type, axes and key values; repeated evidence/limitation paragraphs and modal boilerplate have been removed. A short screen-reader description retains the essential axis and statistical meaning of each new graph. Source links, route cautions and model qualifications remain in the selected-profile details, whose three new timing summaries are also shortened. The cards use a shared compact chart height, combine the reference graph type with the header, and align their height within each row without clipping wrapped text. Morphine ER retains its study mean-and-SD plot and a shorter equivalent numeric label. No SVG coordinates, model parameters, conversion ratios or calculation rules changed. The clinical-content manifest advances to `2026-09-14.4` to identify the revised timing prose; the asset key is `20260914-pk-compact-1`. The cumulative v2.1 note already describes the resulting PK improvements and remains accurate.
