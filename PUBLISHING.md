...
Staging currently opens a temporary eight-logo preview, controlled by `staging/release.json`. Its working calculator remains unchanged inside the preview. See `staging/logo-preview/README.md` for the three routes, explicit artifact allowlist, and restoration instructions. Verify the switcher and gallery as well as the calculator while enabled. Setting `logoPreview` to `false` and deploying restores the normal calculator route and removes the preview assets from the artifact.

1. Inspect git status and remotes. Complete the syntax checks and browser checks in AGENTS.md. Review the single `#version-notes` disclosure in `public/opioidcalculator.html` against frozen v1.0 and the final release state. Maintain cumulative net differences: rewrite/merge existing items, omit intermediate changes, and remove differences reverted to baseline (2 → 5 → 7 → 11 is recorded as 2 → 11). Concise, broad design summaries are welcome; avoid specific or verbose descriptions of individual styling tweaks. Routine refinements do not require updating an existing broad summary. Substantial changes to page organization, workflow order, or how people use the tools warrant a concise update. Continue recording functional fixes and other substantive changes. Include necessary note updates with the corresponding implementation commit, following AGENTS.md; keep historical records intact.
