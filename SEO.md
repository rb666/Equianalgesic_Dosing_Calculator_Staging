...
- Static usage guide explaining the tools, inputs, units, and references. As refined on 2026-09-14, it follows the conversion references in a small native disclosure labelled "How to use this calculator," closed by default. Users can open it with a pointer or keyboard without JavaScript. All guide text and source links remain in the initial HTML; none are loaded on demand or served only to crawlers. Titles, canonical metadata, sharing metadata, and structured data remain unchanged by this reorganization.
...
- Regression checks cover metadata identity, schema relationships, omitted unverified identity/review fields, resolvable fragment links, result snippet exclusions, and safe activation of known calculator links. The guide's disclosure checks require its text and links in the initial HTML, an accessible native summary, a closed initial state, and placement after the conversion references.
...
At the initial SEO release on 2026-09-13, all 32 Node regression tests, JavaScript syntax checks, and artifact preparation passed. Browser checks covered guide readability, desktop/mobile layout, direct tool navigation, keyboard access, and retained calculator behavior. That SEO release left approved clinical tables, core calculations, safety policies, and the clinical-data digest unchanged. Later calculator changes and their validation are recorded separately; this document does not extend that original verification claim to them.

The 2026-09-14 disclosure refinement retains the authored guide content and its links. It reduces the guide's initial visual prominence without removing its HTML content. Google's [hidden-text policy](https://developers.google.com/search/docs/essentials/spam-policies#hidden-text-and-link-abuse) permits accordions used for the visitor's experience, and its [mobile indexing guidance](https://developers.google.com/search/docs/crawling-indexing/mobile/mobile-sites-mobile-first-indexing) recommends retaining content in accordions or tabs when appropriate. Static regression checks cover the disclosure structure and content; desktop/mobile keyboard and layout checks remain part of release verification. This organization change does not guarantee crawling, indexing, ranking, or traffic improvements. Staging remains excluded from indexing.
