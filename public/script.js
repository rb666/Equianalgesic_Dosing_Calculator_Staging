...
const sourceReferences = [
  ...
  },
  {
    ...
    note:
      "Background source for IV codeine and additional route-level OME cross-checks. This calculator still preserves selected local conversion relationships where requested.",
  },
  {
  ...
  },
  {
    ...
    note:
      "Background source for renal cautions and alternative opioid groupings; the eGFR rules in this calculator follow the client-requested configuration.",
  },
  {
    title: "Use of the Child-Pugh Score in Liver Disease – StatPearls",
    url: "https://www.ncbi.nlm.nih.gov/books/NBK542308/",
    note:
      "Background source for liver function markers. This calculator leaves mild, moderate, or severe hepatic classification to clinical judgment rather than calculating a lab-derived score.",
  },
  {
  ...
  },
  {
    ...
    url: "",
    note:
      "This calculator preserves the local IV morphine baseline and legacy hydromorphone or meperidine values while adding the requested oral methadone 4.7 MME factor, 3.0 conservative oral methadone estimate, and hepatic advisory bands.",
  },
  {
...
const updateRenalBandNote = () => {
  ...
  if (band.id === "over50") {
    renalBandNote.textContent =
      "eGFR above 50 mL/min: this calculator applies no automatic renal reduction.";
    return;
  }
  ...
  renalBandNote.textContent =
    "eGFR below 30 mL/min: this calculator marks morphine, codeine, and meperidine as avoid and highlights alternative opioid groups.";
};
...
const getRenalAdvice = ({
  ...
  if (band.id === "over50") {
    return {
      ...
      body:
        "No automatic renal dose reduction is configured in this calculator above 50 mL/min." +
        currentNote,
  ...
  if (band.id === "30to50") {
    ...
    if (!isMMeMode && targetOption) {
      ...
      if (renalModerateAlternativeMedications.has(targetOption.medication)) {
        return {
          ...
          body:
            "No explicit percentage reduction is auto-applied for oxycodone or hydromorphone in this calculator. Use lower starting doses and cautious titration in renal dysfunction." +
            currentNote,
      ...
      if (renalMinimalAlternativeMedications.has(targetOption.medication)) {
        return {
          ...
          body:
            "Methadone, fentanyl, and buprenorphine are listed here as lower-kidney-effect alternatives. No automatic percentage reduction is applied in this calculator, but monitoring and formulation review remain necessary." +
            currentNote,
    ...
    return {
      summary: `Renal: ${band.label}; ${reductionLabel}`,
      title: "Renal rule for eGFR 30-50 mL/min",
      body:
  ...
  if (!isMMeMode && targetOption) {
    if (renalRestrictedMedications.has(targetOption.medication)) {
      return {
        ...
        body:
          "Morphine, codeine, and meperidine are marked as avoid in this calculator below 30 mL/min. Suggested alternatives: oxycodone or hydromorphone with caution, or methadone, fentanyl, or buprenorphine with specialist review and monitoring." +
          currentNote,
    ...
    if (renalMinimalAlternativeMedications.has(targetOption.medication)) {
      return {
        ...
        title: `${targetOption.label} is a lower-kidney-effect alternative`,
        body:
          "Methadone, fentanyl, and buprenorphine are highlighted here as lower-kidney-effect alternatives. No automatic percentage reduction is added in this calculator, but specialist review and close monitoring remain important." +
          currentNote,
        resultLabel: "Preferred class",
      };
    }
  ...
  return {
    ...
    title: "Renal rule for eGFR <30 mL/min",
    body:
      "Morphine, codeine, and meperidine are marked as avoid below 30 mL/min in this calculator. Suggested alternatives: oxycodone or hydromorphone with caution, or methadone, fentanyl, or buprenorphine with specialist review and monitoring." +
      currentNote,
    resultLabel: "Guidance only",
  };
};
